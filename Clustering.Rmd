...
```{r Installations, warning=TRUE, include=FALSE}

source("src/Packages.R")

```

_Function to make binary variables_
```{r Binary function}

makeBin <- function(x, y) {
  x[, y] <- as.factor(x[, y])
  for (nrow in 1:nrow(x)) {
    if (x[nrow, y] == "no") {
      x[nrow, y] <- 1
    } else if (x[nrow, y] == "yes") {
      x[nrow, y] <- 2
    } else if (is.na(x[nrow, y])) {
      x[nrow, y] <- NA
    } else {
      warning(paste0("No yes/no/NA data in row ", nrow))
    }
  }
  return(x)
}
...
data <- read_delim("data/raw/data.csv", ";",
                   escape_double = FALSE, 
...
data <- makeBin(data, "Hypertension")
data <- makeBin(data, "Isch")
data <- makeBin(data, "CABG")
data <- makeBin(data, "Diabetes")
